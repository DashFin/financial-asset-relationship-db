...
class ContextChunker:
    ...
    def __init__(self, config_path: str = ".github/pr-agent-config.yml"):
        ...
        logger.info(
            f"ContextChunker initialized with max_tokens={self.chunking_config.max_tokens}"
        )
            ...
            if total_tokens + content_tokens <= self.chunking_config.max_tokens:
                ...
                if remaining_tokens > self.chunking_config.truncation_threshold:
                    ...
                    available_for_content = (
                        remaining_tokens - 50
                    )  # Reserve for truncation message
...
def main() -> None:
    ...
    try:
        ...
        if not input_data.strip():
            ...
            print(
                json.dumps({"content": "", "chunked": False, "original_keys": []}),
                file=sys.stdout,
            )
            return
