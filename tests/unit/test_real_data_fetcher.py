...
class TestRealDataFetcherInitialization:
    ...
    def test_init_with_all_parameters(self):
        ...
        fallback = lambda: AssetRelationshipGraph()
...
class TestCacheOperations:
    ...
    def test_create_real_database_loads_from_cache_when_exists(self, temp_cache_file, sample_graph):
...
class TestNetworkFetching:
    ...
    def test_create_real_database_uses_fallback_when_network_disabled(self):
        ...
        fetcher = RealDataFetcher(
            enable_network=False,
            fallback_factory=lambda: custom_graph
        )
...
class TestRealDatabaseCreation:
    ...
    def test_create_real_database_success_workflow(
        ...
        # Setup mocks
        mock_equities.return_value = [Equity(
            id="AAPL", symbol="AAPL", name="Apple",
            asset_class=AssetClass.EQUITY, sector="Tech", price=150.0
        )]
        mock_bonds.return_value = [Bond(
            id="TLT", symbol="TLT", name="Treasury",
            asset_class=AssetClass.FIXED_INCOME, sector="Government",
            price=110.0, maturity_date=None, coupon_rate=0.03,
            credit_rating="AAA"
        )]
        mock_commodities.return_value = [Commodity(
            id="GLD", symbol="GLD", name="Gold",
            asset_class=AssetClass.COMMODITY, sector="Metals",
            price=1800.0, commodity_type="Precious Metal", unit="oz"
        )]
        mock_currencies.return_value = [Currency(
            id="EURUSD", symbol="EURUSD=X", name="EUR/USD",
            asset_class=AssetClass.CURRENCY, sector="FX",
            price=1.2, base_currency="EUR", quote_currency="USD"
        )]
        mock_events.return_value = [RegulatoryEvent(
            id="REG001",
    ...
    def test_create_real_database_saves_to_cache_on_success(self):
        ...
        try:
            with patch.object(RealDataFetcher, '_fetch_equity_data') as mock_equities, \
                 patch.object(RealDataFetcher, '_fetch_bond_data') as mock_bonds, \
                 patch.object(RealDataFetcher, '_fetch_commodity_data') as mock_commodities, \
                 patch.object(RealDataFetcher, '_fetch_currency_data') as mock_currencies, \
...
class TestErrorHandling:
    ...
    def test_create_real_database_with_all_fetch_failures(self):
        ...
        with patch.object(RealDataFetcher, '_fetch_equity_data') as mock_eq, \
...
class TestCachePersistence:
    ...
    def test_cache_save_uses_tempfile_for_atomicity(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('src.data.real_data_fetcher.tempfile.NamedTemporaryFile') as mock_temp:
                ...
                with patch.object(RealDataFetcher, '_fetch_equity_data', return_value=[]), \
                     ...
                     patch.object(RealDataFetcher, '_create_regulatory_events', return_value=[]):
