            ...
            for i, line in enumerate(lines, start=1):
                ...
                for pattern in dangerous_patterns:
                    if pattern in line:
                        ...
                        if valid_refs:
                            # Mask valid secret reference spans, then check remaining text
                            masked = list(line)
                            for m in valid_refs:
                ...
                for job_name, job_data in jobs.items():
                    ...
                    for step in steps:
                        if step.get("uses", "").startswith("actions/checkout"):
                            ...
                            # Fix for catastrophic syntax error in original file:
                            if (
                                ref
...
class TestWorkflowBestPractices:
    ...
    def test_actions_use_stable_versions(self, workflow_data):
        """Actions should not use unstable versions like @main or @master."""
        for filename, data in workflow_data.items():
            jobs = data.get("jobs", {})
