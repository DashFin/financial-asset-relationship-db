...
class TestWorkflowMaintainability:
    ...
    def test_workflow_reasonable_size(self, workflow_file: Path):
        """
        Test that a workflow file does not exceed reasonable size limits.

        Emits a warning for files larger than 10 KB and fails if the file is 50 KB or larger,
        suggesting splitting into multiple workflows or using reusable workflows.
        """
...
class TestWorkflowAdvancedValidation:
    ...
    def test_job_dependencies_are_acyclic(self, workflow_file: Path):
        ...
        def has_cycle(node, visited, rec_stack):
            """Detects cycles in the dependency graph starting from the given node.

            Args:
                node: the starting node to check for cycles.
...
class TestWorkflowEnvironmentVariables:
    ...
    def test_env_vars_use_consistent_naming(self, workflow_file: Path):
        ...
        def check_env_names(env_dict):
            """Check that environment variable names follow UPPER_CASE convention."""
