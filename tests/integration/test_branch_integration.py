...
class TestRemovedFilesIntegration:
    ...
    def test_label_workflow_doesnt_need_labeler_config(self):
        ...
        Checks that .github/workflows/label.yml (if present) defines the 'label' job's first step using 'actions/labeler',
        and that the step either omits 'config-path' or sets it to '.github/labeler.yml'. Skips the test if label.yml is missing.
        """
