...
class TestGreetingsWorkflow:
    ...
    def test_greetings_workflow_structure(self, greetings_workflow: Dict[str, Any]):
        ...
        assert (
            "issues" in greetings_workflow["on"]
            or "pull_request" in greetings_workflow["on"]
        )
        assert "jobs" in greetings_workflow
        assert "greeting" in greetings_workflow["jobs"]

    def test_greetings_uses_first_interaction_action(
    ...
    def test_greetings_has_appropriate_messages(
        self, greetings_workflow: Dict[str, Any]
    ):
        ...
        for step in steps:
            if "uses" in step and "first-interaction" in step["uses"]:
                ...
                assert issue_msg and len(issue_msg) > 10, (
                    "Issue message should be meaningful"
                )
...
class TestLabelerWorkflow:
    ...
    def test_labeler_workflow_structure(self, labeler_workflow: Dict[str, Any]):
        ...
        trigger_key = "on"
        assert (
            "pull_request_target" in labeler_workflow[trigger_key]
            or "pull_request" in labeler_workflow[trigger_key]
        )
        assert "jobs" in labeler_workflow

    def test_labeler_has_appropriate_permissions(
        self, labeler_workflow: Dict[str, Any]
    ):
    ...
    def test_labeler_uses_labeler_action(self, labeler_workflow: Dict[str, Any]):
        ...
        for job_name, job_config in labeler_workflow["jobs"].items():
            ...
            if "labeler" in job_name.lower():
                assert action_found, (
                    f"Labeler job '{job_name}' should use labeler action"
                )
            ...
            for step in steps:
                if "uses" in step and "apisec" in step["uses"].lower():
                    ...
                    if "with" in step:
                        step_str = str(step)
                        assert (
                            "apisec_username" in step_str
                            or "username" in step_str.lower()
                        )

    def test_apisec_simplified_no_credential_check(
...
class TestPRAgentWorkflow:
    ...
    def test_pr_agent_no_duplicate_setup_python(
    ...
    def test_pr_agent_has_python_and_node_setup(
        self, pr_agent_workflow: Dict[str, Any]
    ):
    ...
    def test_pr_agent_simplified_context_handling(
        self, pr_agent_workflow: Dict[str, Any]
    ):
...
class TestWorkflowSecurityBestPractices:
        ...
        for workflow_name, workflow_data in all_workflows.items():
            ...
            for job_name, job_config in jobs.items():
                ...
                for step in steps:
                    if "uses" in step:
                        ...
                        if any(
                            ...
                            for keyword in ["comment", "label", "issue", "pr"]
                        ):
                ...
                if needs_permissions:
                    ...
                    has_perms = (
                        "permissions" in job_config or "permissions" in workflow_data
                    )
    ...
    def test_workflows_dont_expose_secrets_in_logs(
        self, all_workflows: Dict[str, Dict[str, Any]]
    ):
...
class TestWorkflowPerformance:
    ...
    def test_workflows_have_reasonable_timeouts(
        self, all_workflows: Dict[str, Dict[str, Any]]
    ):
...
class TestRequirementsDevChanges:
    ...
    def test_pyyaml_is_present(self, requirements_dev: List[str]):
        ...
        pyyaml_found = any(
            "pyyaml" in req.lower() or "yaml" in req.lower() for req in requirements_dev
        )

        assert pyyaml_found, (
            "PyYAML should be in requirements-dev.txt for workflow testing"
        )
