...
class TestWorkflowSyntaxValidation:
    ...
    def test_pr_agent_uses_specific_action_versions(self, pr_agent_workflow):
        ...
        for job_name, job_config in jobs.items():
            ...
            for step in steps:
                ...
                assert version and version.lower() not in {
                    "main",
                    "master",
                    "latest",
                    "head",
                ...
                assert not re.fullmatch(r"[A-Za-z]+", version) or re.fullmatch(
                    r"[0-9a-fA-F]{7,40}", version
                ), (
    ...
    def test_pr_agent_secrets_properly_referenced(self, pr_agent_workflow):
        ...
        secrets_refs = re.findall(
            r"\$\{\{\s*secrets\.([A-Z0-9_]+)\s*\}\}", combined, flags=re.IGNORECASE
        )
        github_token_refs = re.findall(
            r"\$\{\{\s*github\.token\s*\}\}", combined, flags=re.IGNORECASE
        )

        assert ("GITHUB_TOKEN" in [s.upper() for s in secrets_refs]) or (
            len(github_token_refs) > 0
        ), (
    ...
    def test_pr_agent_install_steps_have_error_handling(self):
        ...
        for step in install_steps:
            ...
            if "pip install" in lower_script and "-r" in lower_script:
                ...
                uses_requirements = (
                    " -r " in lower_script or "--requirement" in lower_script
                )
                assert uses_requirements, (
                    "Install must reference a requirements file via -r/--requirement."
                )
                assert guarded or uses_requirements, (
                    "Install steps should guard file existence or safely use pip with -r."
                )
...
class TestWorkflowStepOrdering:
    ...
    def test_pr_agent_checkout_before_setup(self):
        ...
        checkout_idx = next(
            (i for i, name in enumerate(step_names) if "Checkout" in name), None
        )
        setup_idx = next(
            (i for i, name in enumerate(step_names) if "Setup" in name), None
        )
    ...
    def test_pr_agent_install_before_run(self):
        ...
        install_indices = [i for i, name in enumerate(step_names) if "Install" in name]
        run_indices = [
            i
...
class TestWorkflowEnvironmentVariables:
    ...
    def test_github_token_properly_scoped(self):
        ...
        token_uses = re.findall(
            r"GITHUB_TOKEN:\s*\$\{\{\s*secrets\.GITHUB_TOKEN\s*\}\}", content
        )
        ...
        for workflow_path in modified_workflows:
            ...
            for pattern in secret_patterns:
                ...
                dangerous_matches = [
                    m
                    for m in matches
                    if "secrets." not in m and "GITHUB_TOKEN" not in m
                ]
