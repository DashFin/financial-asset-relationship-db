            ...
            for line in f:
                ...
                if not m_name:
                    raise AssertionError(
                        f"Malformed requirement line (invalid package name): {line}"
                    )
                pkg = m_name.group(1)
        ...
        for pkg, ver_spec in requirements:
            if ver_spec:
                try:
                    ...
                    assert False, (
                        f"Invalid version specifier for {pkg}: {ver_spec} ({e})"
                    )
...
class TestRequirementsFileFormatting:
    ...
    def test_pyyaml_and_types_on_separate_lines(self):
        ...
        pyyaml_lines = [
            line
            for line in lines
            if line.startswith("PyYAML") or line.startswith("types-PyYAML")
        ]
...
class TestRequirementsPackageIntegrity:
    ...
    def test_no_duplicate_package_names(self):
        ...
        assert REQUIREMENTS_FILE.exists(), "requirements-dev.txt not found"
        requirements = parse_requirements(REQUIREMENTS_FILE)
        duplicates = TestRequirementsPackageIntegrity._find_duplicate_packages(
            requirements
        )
        assert len(duplicates) == 0, (
...
class TestPyYAMLIntegration:
    ...
    def test_pyyaml_addition_has_both_runtime_and_types(self):
        ...
        types_entry = next(
            (ver for pkg, ver in requirements if pkg == "types-PyYAML"), None
        )
