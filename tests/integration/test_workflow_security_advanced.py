                    ...
                    if "run" in step:
                        ...
                        if "curl" in run_command.lower():
                            ...
                            assert "github.event" not in run_command or (
                                "validate" in run_command.lower()
                                or "sanitize" in run_command.lower()
                            ), (
            ...
            for job_name, job_config in jobs.items():
                ...
                for step_idx, step in enumerate(steps):
                    if "github-script" in step.get("uses", "") or "script" in step:
                        ...
                        if (
                            "createComment" in script_content
                            or "create_comment" in script_content
                        ):
...
class TestWorkflowIsolationAndSandboxing:
    ...
    def test_pull_request_workflows_use_safe_checkout(self, all_workflows):
        ...
        """
        for workflow in all_workflows:
            triggers = workflow["content"].get("on", {}) or workflow["content"].get(
                '"on"', {}
            )
            ...
            if "pull_request_target" in triggers:
                ...
                for job_name, job_config in jobs.items():
                    ...
                    for checkout_step in checkout_steps:
                        ...
                        assert "with" in checkout_step and "ref" in checkout_step.get(
                            "with", {}
                        ), (
        ...
        for workflow in all_workflows:
            ...
            for job_name, job_config in jobs.items():
                if "container" in job_config:
                    ...
                    is_trusted = any(
                        container_image.startswith(registry)
                        for registry in trusted_registries
                    )
                    is_official = (
                        ":" in container_image
                        and "/" not in container_image.split(":")[0]
                    )
